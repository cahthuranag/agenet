...
def _main():
    ...
    parser.add_argument(
        "--d", type=float, nargs="+", default=[100], help="Distance(s) in meters"
    )
    parser.add_argument(
        "--N0", type=float, nargs="+", default=[1e-13], help="Noise power(s) in Watts"
    )
    parser.add_argument(
        "--fr", type=float, nargs="+", default=[6e6], help="Frame rate(s) in Hz"
    )
    parser.add_argument(
        "--num_events", type=int, nargs="+", default=[50], help="Number of events"
    )
    parser.add_argument(
        "--num_nodes", type=int, nargs="+", default=[5], help="Number of nodes"
    )
    parser.add_argument(
        "--active_prob", type=float, nargs="+", default=[0.2], help="Active probability"
    )
    parser.add_argument(
        "--n", type=int, nargs="+", default=[150], help="Total number of qubits"
    )
    parser.add_argument(
        "--k", type=int, nargs="+", default=[50], help="Number of information qubits"
    )
    parser.add_argument(
        "--P", type=float, nargs="+", default=[8e-2], help="Depolarizing probability"
    )
    parser.add_argument(
